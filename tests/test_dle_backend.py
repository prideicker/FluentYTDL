...
class TestDLEProvider:
    ...
    def test_browser_resolution_user_path(self):
        """Invalid user path should be rejected (fallback may still find a browser)."""
        from fluentytdl.auth.providers.dle_provider import DLEProvider
        
        provider = DLEProvider()
        
        # Non-existent user path should not be used (falls back to auto-detect)
        bad_path = "/nonexistent/path/browser.exe"
        result = provider._resolve_browser(bad_path, "edge")
        
        # The result should NOT be the invalid path
        if result is not None:
            assert str(result) != bad_path, "Should not return the invalid user path"
            print(f"  PASS: Invalid user path rejected, fell back to {result}")
        else:
            print("  PASS: Invalid user path rejected, no fallback browser found")
    ...
    def test_browser_resolution_fallback(self):
        ...
        for bt in ["edge", "chrome", "brave", "vivaldi", "opera"]:
            result = provider._resolve_browser(None, bt)
