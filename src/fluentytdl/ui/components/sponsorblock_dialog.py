...
class SponsorBlockCategoriesDialog(MessageBoxBase):
    ...
    def __init__(
        ...
        self.infoLabel = BodyLabel(
            "勾选下方要自动跳过的广告片段类型，这些片段将在下载时自动移除或标记为章节。",
            self
        )
        ...
        for cat_id, cat_name, cat_desc in common_categories:
            category_widget = CategoryCheckBox(cat_id, cat_name, cat_desc, cat_id in current_set, self)
    ...
    def accept(self) -> None:  # type: ignore[override]
        ...
        self.selected_categories = [
            cat_id for cat_id, widget in self.checkboxes.items()
            if widget.isChecked()
        ]
        super().accept()
